...
if __name__ == '__main__':
    ...
    avgImg, FaceVector, diffTrain = eigen_face(train_images, 0.85)

    label = [[i, i, i, i, i] for i in range(40)]
    label = reshape(label, (200, 1))
    te = FaceVector.T * diffTrain
    clf = LinearDiscriminantAnalysis(n_components=39)

    clf.fit(te.T, label.ravel())
    ...
    for i in range(40):
        ...
        for j in range(5):
            ...
            if res == i:
                print(images[i * 10 + 5 + j], "识别结果：", "Ⅰ")
                count += 1
            else:
                print(images[i * 10 + 5 + j], "识别结果:", "Ⅱ(error)")
        total_count += count
